...
class BSpline : public Spline {
	...
	virtual float b2(float u) const {
		return(-3 * u * u * u + 3 * u * u + 3 * u + 1) * (1.0f / 6.0f);
	}
	virtual float b3(float u) const {
	...
	virtual float b2diff(float u) const { return -1.5f * u * u + u+.5f; };
	virtual float b3diff(float u) const { return .5f * u * u; };

public:
	BSpline(const std::vector<glm::vec3>& points, bool loop = false) : Spline(points, loop){
	}
};
